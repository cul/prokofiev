...
<!-- Sheet one from Series VII saved as xlxs
     Edited column headers to a single row
     Exported as csv
     Import into Oxygen, convert headers to tags 
     
     Deleted records SPA_ID="Not found" 
...
<xsl:template match="row">
    ...
    <xsl:text>&#xa;</xsl:text>
    <xsl:comment>EAD <xsl:value-of select="SPA_ID"/>; Goldsmiths_ID <xsl:value-of select="Goldsmiths_ID"/>
    </xsl:comment>
    <xsl:text>&#xa;</xsl:text>
 ...
 <c level="file">
     <did>
         ...
         <note>
             ...
             <p>Name of subject(s): <xsl:value-of select="Name_of_subject"/></p><xsl:text>&#xa;</xsl:text>
