...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="outputxml">
        <xsl:for-each select="RECORD[TYPE_OF_WORK='Video Tape']">
            
            

        
        <xsl:variable name="Recordnum">
            <xsl:number count="RECORD[TYPE_OF_WORK='Video Tape']"></xsl:number>
        </xsl:variable>
        ...
        Type 7 Video Tape starts numbering after DVD:  15292 -->
        
        <xsl:variable name="SPA">
            <xsl:number value="$Recordnum + 15291"/>
        </xsl:variable>
