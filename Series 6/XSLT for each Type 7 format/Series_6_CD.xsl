...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="outputxml">
        <xsl:for-each select="RECORD[TYPE_OF_WORK='CD'] | RECORD[TYPE_OF_WORK='CD (Enhanced CD)']">
            
            

        
        <xsl:variable name="Recordnum">
            <xsl:number count="RECORD[TYPE_OF_WORK='CD'] | RECORD[TYPE_OF_WORK='CD (Enhanced CD)']"></xsl:number>
        </xsl:variable>
        ...
        Type 7 CD starts numbering after Audio Tape:  14731 -->
        
        <xsl:variable name="SPA">
            <xsl:number value="$Recordnum + 14730"/>
        </xsl:variable>
