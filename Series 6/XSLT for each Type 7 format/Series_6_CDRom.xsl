...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="outputxml">
        <xsl:for-each select="RECORD[TYPE_OF_WORK='CDRom']">
            
            
        
        <xsl:variable name="Recordnum">
            <xsl:number count="RECORD[TYPE_OF_WORK='CDRom']"></xsl:number>
        </xsl:variable>
        ...
        Type 7 CDRom starts numbering after LP:  15577
        
                Only 1 CDRom in collection
        -->
        
        <xsl:variable name="SPA">
            <xsl:number value="$Recordnum + 15342"/>
        </xsl:variable>
