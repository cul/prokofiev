...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="outputxml">
        <xsl:for-each select="RECORD[TYPE_OF_WORK='DVD']">
            
            

        
        <xsl:variable name="Recordnum">
            <xsl:number count="RECORD[TYPE_OF_WORK='DVD']"></xsl:number>
        </xsl:variable>
        ...
        Type 7 DVD starts numbering after CD:  15236 -->
        
        <xsl:variable name="SPA">
            <xsl:number value="$Recordnum + 15235"/>
        </xsl:variable>
